		...
		Object.keys(t).forEach(function (i) {
			C(t[i]) &&
				Object.keys(t[i]).forEach(function (s) {
					'function' == typeof t[i][s] && (t[i][s] = t[i][s].on(e));
				}),
				(e[i] = t[i]);
		});
	}
							...
							if (t.slidesPerColumn > 1) {
								...
								if ('row' === t.slidesPerColumnFill && t.slidesPerGroup > 1) {
									...
									'column' === t.slidesPerColumnFill
										...
										: (O = $ - (A = Math.floor($ / P)) * P);
								L.css(
									'margin-' + (this.isHorizontal() ? 'top' : 'right'),
								...
								if ('auto' === t.slidesPerView) {
									...
									if (
										...
										var Y = parseFloat(B.getPropertyValue('width') || 0),
											...
											F = parseFloat(B.getPropertyValue('padding-left') || 0),
				...
				slideTo: function (e, t, i, s) {
					...
					if (n.cssMode) {
						...
						if ((p && (y = u.scrollWidth - u.offsetWidth - y), 0 === t))
							...
							u.scrollTo(
								(((w = {})[b ? 'right' : 'top'] = y), (w.behavior = 'smooth'), w)
							);
			...
			events: {
				attachEvents: function () {
					var e = r(),
						...
						n = this.device,
						l = this.support;
					(this.onTouchStart = O.on(this)),
						(this.onTouchMove = A.on(this)),
						(this.onTouchEnd = D.on(this)),
						t.cssMode && (this.onScroll = B.on(this)),
						(this.onClick = N.on(this));
					var o = !!t.nested;
			...
			setDragPosition: function (e) {
				...
				(t =
					(i.getPointerPosition(e) -
						a.offset()[this.isHorizontal() ? 'right' : 'top'] -
						(null !== l ? l : r / 2)) /
					(n - r)),
...
(function ($) {
	...
	window.elementorFrontendConfig = {
		...
		i18n: {
			shareOnFacebook: 'به اشتراک گذاری در فیسبوک',
			shareOnTwitter: 'به اشتراک گذاری در توییتر',
			pinIt: 'پین کردن',
	...
	jQuery('.helper .link.toggle').on('click', function (e) {
		e.preventDefault();
		var el = jQuery(this);
	...
	function mfn_stickyH() {
		if (jQuery('body').hasClass('header-below')) {
			...
			// default
			if (jQuery('#Action_bar').innerHeight())
				mfn_header_height =
					jQuery('#Top_bar').innerHeight() +
					jQuery('#Action_bar').innerHeight();
			else mfn_header_height = jQuery('#Top_bar').innerHeight();
		}
	}
	// init
	function mfn_sticky() {
	 ...
	 * Overlay menu
	 * --------------------------------------------------------------------------- */
	jQuery('.overlay-menu-toggle').on('click', function (e) {
		e.preventDefault();
	 ...
	 * --------------------------------------------------------------------------- */
	function mfn_header() {
		var rightW = $('.top_bar_left').innerWidth() || 0;
		...
		if (leftW > 0)
			$('.top_bar_right, .menu > li > ul.mfn-megamenu').css('width', leftW);
	}
	...
	function onePageActive() {
		if ($('body').hasClass('one-page')) {
			...
			else var start = windowT + stickyH + 1;
	...
	/* ---------------------------------------------------------------------------
	 * niceScroll | Padding left fix for short content
	 * --------------------------------------------------------------------------- */
	...
	function onePageMenu() {
		if (jQuery('body').hasClass('one-page')) {
			...
			// click
			jQuery('#menu a[data-hash]').on( 'click',function(e) {
	...
	onePageMenu(); */
		...
		function onePageMenu() {
			if ($('body').hasClass('one-page')) {
				...
				// click ----------
				$(document).on('click', '#menu a[data-hash]', function (e) {
					e.preventDefault(); // only with: body.one-page
					...
					if ($('.header-fixed.ab-show #Action_bar').innerHeight())
						var headerFixedAbH = $(
							'.header-fixed.ab-show #Action_bar'
						).innerHeight();
					else var headerFixedAbH = 0;

					if ($(hash).siblings('.ui-tabs-nav').innerHeight())
						var tabsHeaderH = $(hash).siblings('.ui-tabs-nav').innerHeight();
					else var tabsHeaderH = 0;
		...
		function creativeHeader() {
			jQuery('.creative-menu-toggle').on('click', function (e) {
				e.preventDefault();
		...
		jQuery('#popup_contact > a.button').on('click', function (e) {
			e.preventDefault();
		...
		jQuery('#Intro .intro-next').on('click', function (e) {
		...
		jQuery('.sliding-top-control').on('click', function (e) {
			e.preventDefault();
		...
		jQuery('#search_button, #Top_bar .icon_close').on('click', function (e) {
			e.preventDefault();
	...
	if (!isMobileWebkit && jQuery(window).width() >= 768) {
		 ...
		 * Ajax | Load More
		 * --------------------------------------------------------------------------- */
		jQuery('.pager_load_more').on('click', function (e) {
			e.preventDefault();
			var el = jQuery(this);
		...
		jQuery('.portfolio_prev_js').on('click', function (e) {
			e.preventDefault();
			var stickyH = jQuery('#Top_bar.is-sticky').innerHeight();
		 ...
		 * Smooth scroll
		 * --------------------------------------------------------------------------- */
		jQuery('li.scroll > a, a.scroll').on('click', function (e) {
			var url = jQuery(this).attr('href');
			var hash = '#' + url.split('#')[1];
			var stickyH = jQuery('.sticky-header #Top_bar').innerHeight();
			var tabsHeaderH = jQuery(hash).siblings('.ui-tabs-nav').innerHeight();
		...
		jQuery('.mfn-acc .question > .title').on('click', function (e) {
		 ...
		 * Section navigation
		 * --------------------------------------------------------------------------- */
		jQuery('.section .section-nav').on('click', function (e) {
			var el = jQuery(this);
	...
	jQuery(window).on('load', function () {
		 ...
		 * Skills
		 * --------------------------------------------------------------------------- */
		jQuery('.bars_list').each(function () {
			var el = jQuery(this);
			el.waypoint({
				offset: '100%',
				triggerOnce: true,
		 ...
		 * Progress Icons
		 * --------------------------------------------------------------------------- */
		jQuery('.progress_icons').each(function () {
			var el = jQuery(this);
			el.waypoint({
				offset: '100%',
				triggerOnce: true,
		...
		jQuery('.animate-math .number').each(function () {
			var el = jQuery(this);
			el.waypoint({
				offset: '100%',
				triggerOnce: true,
	...
	jQuery(document).on('mouseUp', function (e) {
		...
		if (jQuery('#searchform').has(e.target).length === 0) {
			if (jQuery('#searchform').hasClass('focus')) {
				jQuery(this).find('.icon_close').on('click');
			}
		}
	});
			...
			if ($(this).hasClass('carouFredSel')) {
				...
				if (slider.siblings('.slider_prev').length) {
					...
					slider.slick({
						...
						prevArrow:
							'<a class="button button_js slider_prev" href="#"><span class="button_icon"><i class="icon-right-open-big"></i></span></a>',
						nextArrow:
							'<a class="button button_js slider_next" href="#"><span class="button_icon"><i class="icon-left-open-big"></i></span></a>',
	...
	function mfnSliderPortfolio() {
		/*	$('.portfolio_slider_ul').each(function(){
			...
			slider.slick({
				...
				prevArrow		: '<a class="slider_nav slider_prev themebg" href="#"><i class="icon-right-open-big"></i></a>',
				nextArrow		: '<a class="slider_nav slider_next themebg" href="#"><i class="icon-left-open-big"></i></a>',
			...
			slider.on( 'dragstart', '.slick-slide a[rel="prettyphoto"]', function( event ){
				if( pretty ){
					$(this).addClass( 'unbind-pretty' ).off( 'click.prettyphoto' );
				}
			});
			...
			slider.on( 'afterChange', function( event, slick, currentSlide, nextSlide ){
				if( pretty ){
					$( 'a.off-pretty[rel="prettyphoto"]', slider ).prettyPhoto( pretty ).removeClass( 'unbind-pretty' );
				}
			});
		});
		*/
		...
		$('.portfolio_slider_ul').each(function () {
			...
			slider.slick({
				...
				prevArrow:
					'<a class="slider_nav slider_prev themebg" href="#"><i class="icon-right-open-big"></i></a>',
				nextArrow:
					'<a class="slider_nav slider_next themebg" href="#"><i class="icon-left-open-big"></i></a>',
	...
	jQuery(document).ready(function ($) {
		jQuery('.masonry.isotope,.isotope').isotope();
		jQuery('#configurator .control').on('click', function (e) {
			e.preventDefault();
		...
		jQuery('#mfn-demo-panel .control').on('click', function (e) {
			e.preventDefault();
